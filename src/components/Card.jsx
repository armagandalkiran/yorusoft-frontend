...
const Card = ({ item }) => {
  return (
    <Container type={item.title === "Standard" ? "Standard" : undefined}>
      <RibbonImage
        type={item.title === "Standard" ? "Standard" : undefined}
        src={Ribbon}
      />
      <CardContainer type={item.title === "Standard" ? "Standard" : undefined}>
        ...
        {item.features.map((featureItem, index) => (
          <Features
            key={index}
            color={item.features.length - 1 === index ? "white" : "#EEE8E8"}
          >
            <ImageContainer>
              <FeatureImg alt="img" src={featureItem.img} />
            </ImageContainer>
            <FeatureDesc color={featureItem.color}>
              {featureItem.feature}
            </FeatureDesc>
          </Features>
        ))}
        <Button type={item.title === "Standard" ? "Standard" : undefined}>
          Contact Us
        </Button>
      </CardContainer>
    </Container>
